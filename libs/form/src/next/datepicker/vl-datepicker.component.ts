...
export const dateRangeSeparator = ' tot en met ';
...
export class VlDatepickerComponent extends FormControl {
    ...
    firstUpdated(changedProperties: Map<string, unknown>) {
        ...
        if (changedProperties.has('type') && (!this.format || this.format === datepickerDefaults.format)) {
            const { format: dateFormat } = datepickerDefaults;
    ...
    updated(changedProperties: Map<string, unknown>) {
        ...
        if (
            ['disabled', 'readonly', 'minDate', 'maxDate', 'minTime', 'maxTime'].some((prop) =>
