...
import { Filter } from '../utils/vl-qlik.model';
...
registerWebComponents([VlRegionElement, VlLayoutElement, VlQlikDashboardComponent]);
...
describe('qlik visual without filters', () => {
    beforeEach(() => {
        ...
        cy.mount(html` ${until(render(true), html` <vl-loader data-vl-text>Visualisatie aan het laden</vl-loader>`)}`);
    });

    it('empty filters', () => {
        cy.get('vl-qlik-dashboard').shadow().find('div[is="vl-search-filter"]').should('not.exist');
    });

    it('non empty visuals', () => {
        viz_without_views
            .flatMap((vis) => vis.map((v) => v.id))
            .forEach((vId) => {
...
describe('qlik visual with filters', () => {
    beforeEach(() => {
        ...
        cy.mount(html` ${until(render(false), html` <vl-loader data-vl-text>Visualisatie aan het laden</vl-loader>`)}`);
    });

    it('non empty filters', () => {
        cy.get('vl-qlik-dashboard').shadow().find('div[is="vl-search-filter"]').should('exist');

        filters
            .map((f: Filter) => f.filter.name)
            .forEach((fName) => {
                cy.get('vl-qlik-dashboard')
                    .shadow()
