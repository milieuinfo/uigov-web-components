...
Dit component laat de gebruiker toe om verschillende lagen van de `vl-map` zichtbaar en weer onzichtbaar te maken.
...
<DocsStory id="map-layer-switcher--layer-switcher-default" />

## Gebruik

Deze component wordt typisch in de `vl-map-side-sheet` gebruikt [meer info](?path=/docs/map-side-sheet--map-side-sheet-default).
...
<DocsStory id="map-layer-switcher--layer-switcher-specialised-options" />

### Kaartlagen met resolutie

Kaart met optie om kaartlagen met resolutie te tonen en verbergen

<DocsStory id="map-layer-switcher--layer-switcher-resolution-option" />
...
<Canvas>
    <Story id="map-layer-switcher--layer-switcher-dynamic" inline={false} height={650} />
</Canvas>
