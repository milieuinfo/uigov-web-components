...
Dit component is de volgende versie van de [vl-select](?path=/docs/elements-select--select-default) component en de [vl-multiselect](?path=/docs/elements-multiselect--multiselect-default) component, we raden aan deze versie te gebruiken.<br />

## Voorbeeld

```js
import { VlSelectComponent } from '@domg-wc/components/next/form/select';
```
...
Als je een value programmatorisch wil selecteren moet je voor deze optie de 'selected' boolean op true zetten.

<Canvas of={VlSelectStories.SelectPreSelected} />
