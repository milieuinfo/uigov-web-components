describe('story vl-icon', () => {
    it('should contain an icon', () => {
        cy.visit('http://localhost:8080/iframe.html?id=elements-icon--icon-default&viewMode=story');
        cy.getDataCy('icon-default');
    });

    it("should contain an light calendar icon that's upside down", () => {
        cy.visit(
            'http://localhost:8080/iframe.html?args=light:true;fullRotate:true&id=elements-icon--icon-default&viewMode=story'
        );
        cy.getDataCy('icon-default')
    ...
    it("should contain an icon that's rotated 90 degrees", () => {
        cy.visit('http://localhost:8080/iframe.html?args=rotate:true&id=elements-icon--icon-default&viewMode=story');
        cy.getDataCy('icon-default').should('have.class', 'vl-vi-u-90deg');
    });

    it('should contain a small icon', () => {
        cy.visit('http://localhost:8080/iframe.html?args=size:small&id=elements-icon--icon-default&viewMode=story');
        cy.getDataCy('icon-default').should('have.class', 'vl-icon--small');
    });

    it('should contain a large icon', () => {
        cy.visit('http://localhost:8080/iframe.html?args=size:large&id=elements-icon--icon-default&viewMode=story');
        cy.getDataCy('icon-default').should('have.class', 'vl-icon--large');
    });

    it('should contain an icon before an element with text', () => {
        cy.visit('http://localhost:8080/iframe.html?id=elements-icon--icon-before&viewMode=story');
    ...
    it('should contain an icon before an element', () => {
        cy.visit('http://localhost:8080/iframe.html?id=elements-icon--icon-after&viewMode=story');
